...
## VERSIONES ##
# 'v1.3'
# Corregidos errores con la búsqueda y salida a pantalla de la última ALERTA
# Función 'imprimir_alerta ()' cambia salida según tiene o no argumentos.
# 'v1.2'
...
# Funciona porque el primer resultado es el archivo con la fecha de modificación más reciente. Sino habría que ordenarlos.
# find /home/pi/logs/temp_alertas/ -type t -name "*.log" -print | sort -r | head -n 1
  file_ULTIMAALERTA=$(find /home/pi/logs/temp_alertas/ -type f -name "*.log" -print -quit)
...
function imprimir_alerta () {
  printf "\nÚltima ALERTA:\n"
  [[ -z $1 ]] && echo $( basename $file_ULTIMAALERTA ) || cat "$file_ULTIMAALERTA"
}
